...
class IndustryModel(TFTBatchModel):
    def __init__(
        self,
    ...
    def _build_train_dataset(
        ...
        # 透传行业分类和股票映射关系，后续使用
        self.sw_ins_mappings = ds.sw_ins_mappings
        return ds      
    ...
    def _create_model(self, train_sample) -> nn.Module:
        ...
        model = MlpTogeModule(
            ...
            sw_ins_mappings=self.sw_ins_mappings,
...
class DateTogeModel(TogeModel):
    ...
    def _build_train_dataset(
        ...
        """使用日期整合的数据集作为训练和测试数据集"""
        
        # 使用日期汇聚数据集
        if mode=="train":
            ds = DateShiftedDataset(
                target_series=target,
