...
class _TFTCusModule(_TFTModule):
    ...
    def validation_step(self, val_batch_ori, batch_idx) -> torch.Tensor:
        ...
        import_vr_acc,import_recall,import_price_acc,import_price_nag,price_class,import_index,out_last_raise_acc,out_last_raise_rate \
             = self.compute_real_class_acc(target, target_vr_class,vr_class_certainlys=vr_class_certainlys,
                output_inverse=output_inverse,slope_out=slope_out.cpu().numpy(), past_covariate=past_covariate, \
    ...
    def compute_vr_class_acc(self,vr_class,vr_target_from_ds,target_info=None,output_inverse=None,last_target_vr_class=None,last_vr_class_certainlys=None):
        ...
        # 数值上也需要有上涨幅度
        import_index_bool = import_index_bool & (torch.sum(slope_out,dim=1)>3)
        ...
        sec_acc_count = torch.sum(vr_target[sec_index]==CLASS_SIMPLE_VALUE_SEC)
        if sec_index.shape[0]==0:
            sec_acc = torch.tensor(0.0)
        else:
            sec_acc = sec_acc_count/sec_index.shape[0]
                     
        # 重点类别的召回率    
        total_imp_cnt = torch.sum(vr_target==CLASS_SIMPLE_VALUE_MAX)
        total_sec_cnt = torch.sum(vr_target==CLASS_SIMPLE_VALUE_SEC)
        if total_imp_cnt!=0:
            import_recall = import_acc_count/total_imp_cnt
            sec_recall = sec_acc_count/total_sec_cnt
        else:
            import_recall = 0
            sec_recall = 0

        # 重点类别的价格准确率
        price_class,raise_range = compute_price_class_batch(focus_target[:,self.input_chunk_length:],mode="fast")
        price_class = torch.Tensor(price_class).int()
        import_price_acc_count = torch.sum(price_class[import_index]==CLASS_SIMPLE_VALUE_MAX)
        sec_price_acc_count = torch.sum((price_class[sec_index]==CLASS_SIMPLE_VALUE_MAX) | (price_class[sec_index]==CLASS_SIMPLE_VALUE_SEC))
        import_price_nag_count = torch.sum(price_class[import_index]==0)
        sec_price_nag_count = torch.sum((price_class[sec_index]==0) | (price_class[sec_index]==1))
        if import_index.shape[0]==0:
        ...
        if sec_index.shape[0]==0:
            sec_price_acc = torch.tensor(0.0)
            sec_price_nag = torch.tensor(0.0)
        else:
    ...
    def compute_slope_acc(self,last_vr_class_certainlys,target_info=None,last_target_vr_class=None):
        """统计反转趋势准确率"""
        ...
        # 总体准确率
        total_acc = torch.sum(last_vr_class_certainlys==last_target_vr_class)/last_target_vr_class.shape[0]  
        
        # 上升趋势统计
        pos_index_bool = (last_vr_class_certainlys==1)
        pos_index = torch.where(pos_index_bool)[0]
        pos_acc_cnt = torch.sum(last_target_vr_class[pos_index]==1)
        pos_acc = pos_acc_cnt/pos_index.shape[0]
        pos_recall = pos_acc_cnt/torch.sum(last_target_vr_class==1)
        
        # 下降趋势统计
        neg_index_bool = (last_vr_class_certainlys==0)
        neg_index = torch.where(neg_index_bool)[0]
        neg_acc_cnt = torch.sum(last_target_vr_class[neg_index]==0)
        neg_acc = neg_acc_cnt/neg_index.shape[0]
        neg_recall = neg_acc_cnt/torch.sum(last_target_vr_class==0)
        ...
        price_class = torch.Tensor(price_class).int()
        # 返回准确率，以及上涨下跌占比
        return pos_acc,pos_recall,neg_acc,neg_recall
    ...
    def compute_real_class_acc(self,target_ori,target_vr_class,target_info=None,output_inverse=None,slope_out=None, \
                               vr_class_certainlys=None,last_target_vr_class=None,last_vr_class_certainlys=None,past_covariate=None):
        """计算涨跌幅分类准确度"""
        ...
        rev_boole = np.mean(rev_cov,axis=1)>0.3
        import_index_bool = import_index_bool & rev_boole
        
        # 统计最后一段在上涨趋势下的准确率
        out_last_filter = last_vr_class_certainlys[output_import_index_bool]
        target_last_filter = last_target_vr_class[output_import_index_bool]
        out_last_filter_raise_bool = out_last_filter==1
        out_last_filter_raise_index = np.where(out_last_filter_raise_bool)[0]
        out_last_raise_acc_cnt = np.sum(target_last_filter[out_last_filter_raise_index]==1)
        out_last_raise_acc = out_last_raise_acc_cnt/out_last_filter_raise_index.shape[0]     
        out_last_raise_rate = out_last_filter_raise_index.shape[0]/output_import_index_bool.shape[0]
    ...
    def _compute_loss(self, output, target):
        ...
        for t in target_info:
            ...
            target_slope_list.append(t["raise_range"])                       
