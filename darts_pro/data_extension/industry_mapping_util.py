...
class FuturesMappingUtil:
    ...
    """行业分类和股票映射关系对象数据结构规范
       ...
       其中第1列(indus_index)： 行业分类对应的序列（TargetSeries）排序号（排序号非连续），类型为int
          ...
          第9列(night_flag)： 对应的夜盘标志，数组长度需要与第1列中的数组长度一致，数组元素类型为int
          第10列(magin_radio)： 对应的交易保证金标志，数组长度需要与第1列中的数组长度一致，数组元素类型为int
          第11列(create_year)： 对应的品种创建年份标志，数组长度需要与第1列中的数组长度一致，数组元素类型为int
    """                   
    ...
    def build_accord_mapping(target_series,fur_indus_df=None,instrument_df=None,dataset=None):
        ...
        for fur_code,group in instrument_df.groupby("indus_code"):
            ...
            night_flag = group["night_flag"].values
            magin_radio = group["magin_radio"].values
            create_year = group["create_year"].values
            instrument_index = np.array([keep_index[keep_index[:,0]==ins_code][0,1] for ins_code in instrument_codes])
            # 品种编码数组长度和序号数组长度需要相等
            raise_if_not(
                instrument_index.shape[0] == instrument_codes.shape[0],
            ...
            fur_ins_mapping = [industry_index,fur_code,group[g_col].values,range_index,instrument_codes,instrument_names,industry_name,exchange_id,night_flag,magin_radio,create_year]
        ...
        if total_index!=-1:
            fur_ins_mapping = [total_index,'ZS_ALL',combine_content[sort_index,0].astype(np.int),
                        combine_content[sort_index,0].astype(np.int),combine_content[sort_index,3],combine_content[sort_index,1],'综合',combine_content[sort_index,4],combine_content[sort_index,5],combine_content[sort_index,6],combine_content[sort_index,7]]
            fur_ins_mappings.append(fur_ins_mapping)
    ...
    @staticmethod
    def get_industry_codes(sw_ins_mappings,without_main=True):
        
        industry_codes = sw_ins_mappings[:,1]
        if without_main:
            main_index = FuturesMappingUtil.get_main_index_in_indus(sw_ins_mappings)
            industry_codes = np.concatenate([industry_codes[:main_index],industry_codes[main_index+1:]])        
    ...
    @staticmethod
    def get_night_flag_ids(sw_ins_mappings):
        main_index = FuturesMappingUtil.get_main_index_in_indus(sw_ins_mappings)
        return sw_ins_mappings[main_index,8]
    
    @staticmethod
    def get_magin_radio_flags(sw_ins_mappings):
        main_index = FuturesMappingUtil.get_main_index_in_indus(sw_ins_mappings)
        return sw_ins_mappings[main_index,9]    

    @staticmethod
    def get_create_year_flags(sw_ins_mappings):
        main_index = FuturesMappingUtil.get_main_index_in_indus(sw_ins_mappings)
        return sw_ins_mappings[main_index,10]
    ...
    def get_combine_industry_instrument(sw_ins_mappings):
        ...
        night_flag = sw_ins_mappings[:,8]
        magin_radio = sw_ins_mappings[:,9]
        create_year = sw_ins_mappings[:,10]
        ...
        for i in range(ins_index.shape[0]):
            ...
            item_combine = np.stack([ins_index[i],ins_names[i],indus_code_arr,ins_codes[i],exchange_ids[i],night_flag[i],magin_radio[i],create_year[i]])
