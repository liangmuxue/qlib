...
class FuturesIndustryModule(MlpModule):
    ...
    def create_real_model(self,
        ...
        **kwargs):
            ...
            combine_nodes = FuturesMappingUtil.get_industry_instrument(self.train_sw_ins_mappings)
    ...
    def training_step_real(self, train_batch, batch_idx): 
        ...
        inp = (past_target,past_covariates, historic_future_covariates,future_covariates,static_covariates,price_targets,index_round_targets)     
        ...
        for i in range(self.get_optimizer_size()):
            ...
            loss,detail_loss = self._compute_loss((output,vr_class,tar_class), 
                            (future_target,target_class,future_round_targets,index_round_targets,price_targets),optimizers_idx=i)
    ...
    def validation_step_real(self, val_batch, batch_idx) -> torch.Tensor:
        ...
        loss,detail_loss = self._compute_loss((output,vr_class,vr_class_list), 
                    (future_target,target_class,future_round_targets,index_round_targets,price_targets),optimizers_idx=-1)
        (corr_loss_combine,ce_loss,fds_loss,cls_loss) = detail_loss
    ...
    def on_validation_epoch_end(self):
        ...
        if self.mode is not None and self.mode.startswith("pred_") :
            ...
            output_3d,past_target_3d,future_target_3d,target_class_3d,price_targets_total, \
                future_round_targets_total,index_round_targets_3d,target_info_3d = self.combine_output_total(self.output_result)
            ...
            for index in range(target_class_3d.shape[0]):
                ...
                for j in range(len(self.past_split)):
                    ...
                    for k,instruments in enumerate(ins_with_indus):
                        ...
                        price_array_range = price_array_norm[:,-4] - price_array_norm[:,-self.output_chunk_length-1]     
                    ...
                    indust_target = index_round_targets[...,j]
                    infer_indust_target = infer_index_round_targets[...,j]
                    indust_output_differ_value = ce_output[j][index] - infer_indust_target
                    win = "indus_round_target_{}_{}".format(j,viz_total_size)
                    target_title = "industry compare_{},mean_tar:{}_{},date:{}".format(j,round(ce_output[2][index].mean(),2),round(index_round_targets[...,2].mean(),2),date)
                    view_data = np.stack([indust_output_value,indust_target,indust_output_differ_value,price_range_total]).transpose(1,0)
                    if j in DRAW_SEQ:
                        tar_viz.viz_bar_compare(view_data,win=win,title=target_title,rownames=indus_names.tolist(),legends=["pred","target","differ_pred","price"])   
    ...
    def dump_val_data(self,val_batch,outputs,detail_loss):
        ...
        output_res = (output,choice_out.cpu().numpy(),trend_value.cpu().numpy(),combine_index.cpu().numpy(),past_target.cpu().numpy(),
                      ...
                      price_targets.cpu().numpy(),future_round_targets.cpu().numpy(),index_round_targets.cpu().numpy(),target_info)
    ...
    def combine_output_total(self,output_result):
        ...
        for item in output_result:
            (output,choice,trend_value,combine_index,past_target,future_target,target_class,price_targets,future_round_targets,index_round_targets,target_info) = item
            x_bar_inner = []
            sv_inner = []
            cls_inner = []
            ce_index_inner = []
        ...
        return (x_bar_total,sv_total,cls_total,ce_index_total,choice_total,trend_total,combine_index_total), \
                    ...
                    price_targets_total,future_round_targets_total,index_round_targets_total,target_info_total        
    ...
    def combine_result_data(self,output_result=None,predict_mode=False):
        ...
        sw_ins_mappings = self.train_sw_ins_mappings if self.trainer.state.stage==RunningStage.TRAINING else self.valid_sw_ins_mappings
        # 使用全部验证结果进行统一比较
        output_3d,past_target_3d,future_target_3d,target_class_3d,price_targets_3d,_,index_round_targets_3d,target_info_3d  = self.combine_output_total(output_result)
    ...
    def build_import_index(self,output_data=None,target=None,price_target=None,infer_index_round_targets=None,combine_instrument=None,instrument_index=None):  
        ...
        return import_index,overroll_trend,trend_value
