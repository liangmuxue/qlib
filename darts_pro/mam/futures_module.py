...
class FuturesTogeModule(MlpModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.result_columns = ["date","top_index","trend_flag"]
    ...
    def on_validation_epoch_end(self):
        ...
        sw_ins_mappings = self.train_sw_ins_mappings if self.trainer.state.stage==RunningStage.TRAINING else self.valid_sw_ins_mappings
        rate_total,result_total,total_imp_cnt = self.combine_result_data(self.output_result) 
    ...
    def combine_result_data(self,output_result=None,predict_mode=False):
        ...
        for i in range(target_class_3d.shape[0]):
            ...
            container_index = industry_index if self.target_mode in [0,1] else [main_index]
        ...
        rate_columns = ["date"] + ["cls{}_cnt".format(i) for i in range(len(CLASS_SIMPLE_VALUES.keys()))] + \
                ["total_cnt","trend_value","trend_correct"]     
        # 遍历按日期进行评估
        for i in range(target_class_3d.shape[0]):
            ...
            import_index,trend_value,result_list = self.build_import_index(output_data=output_list,target_info=target_info_list,
                            target=whole_target,combine_instrument=combine_content,target_class=target_class_list)
            ...
            # 计算准确率，包括趋势预测准确率
            import_price_result,main_trend_correct = self.collect_result(import_index,overroll_trend=trend_value,
                                                                target_info=target_info_list,result_list=result_list)
        ...
        return rate_total,result_total_list,total_imp_cnt
    
    def collect_result(self,import_index,overroll_trend=0,target_info=None,result_list=None): 
        """收集预测对应的实际数据"""

        sw_ins_mappings = self.train_sw_ins_mappings if self.trainer.state.stage==RunningStage.TRAINING else self.valid_sw_ins_mappings
        all_instruments = FuturesMappingUtil.get_all_instrument(sw_ins_mappings)
        # 对于预测数据，生成对应涨跌幅类别
        import_price_result = []
        ...
        # 新增行业预测结果到原结果集
        result_list["trend_result"] = np.array(trend_results)
        # 计算整体趋势准确率
        main_trend_correct = 0
        total_number = all_instruments.shape[0]
        raise_num = np.sum(total_diff_range>=0)
        if overroll_trend==1 and raise_num>=total_number//2:
            main_trend_correct = 1
        elif overroll_trend==0 and raise_num<=total_number//2:
            main_trend_correct = 1
        else:
            main_trend_correct = 0        
                     
        return import_price_result,main_trend_correct
    
    def build_import_index(self,output_data=None,target=None,target_info=None,combine_instrument=None,target_class=None):  
        """生成涨幅达标的预测数据下标"""
        ...
        import_index,trend_value,result_list = self.strategy_top(cls_values,ce_values,
                                    target=target,target_info=target_info,target_class=target_class)
        ...
        return import_index,trend_value,result_list
                                          
    def strategy_top(self,cls,ce_values,target=None,target_info=None,target_class=None):
        """排名方式筛选候选者"""
        ...
        instrument_all = FuturesMappingUtil.get_all_instrument(sw_ins_mappings)
        ...
        raise_top_index = np.argsort(cls_compare)[:top_num] 
        fall_top_index = np.argsort(-cls_compare)[:top_num]   

        result_list = []
        def combine_results(top_index,trend_value):
            for i in range(top_index.shape[0]):
                idx = top_index[i]
                if target_class[instrument_all[idx]]<0:
                    continue
                result_list.append([date,instrument_all[idx],trend_value])
        
        combine_results(raise_top_index,1)
        combine_results(fall_top_index,0)
        ...
        result_list['top_index'] = result_list['top_index'].astype(int) 
          
        # Mock      
        trend_value = 0
        # 转换为实际的索引
        import_index_real = result_list['top_index'].values
