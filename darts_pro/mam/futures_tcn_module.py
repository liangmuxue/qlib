...
from darts_pro.act_model.tcn_ts import UnionTcnCombine
...
class FuturesTcnModule(MlpModule):
    """期货基于TCN的双向判断的模型"""              
    ...
    def forward(
        ...
        for i, m in enumerate(self.sub_models):
            ...
            if optimizer_idx == i or optimizer_idx >= sub_model_length or optimizer_idx == -1:
                ...
                futures_convs = futures_convs.permute(0,2,3,1)[...,instruments].permute(0,1,3,2)
                static_covs = static_covs.permute(0,2,1)[...,instruments].permute(0,2,1)
                main = torch.cat([past_convs_item,his_future_covs],dim=2)
                out = m(main, static_covs,futures_convs)                
