...
class DinkModule(_TFTModuleBatch):
    """自定义基于图模式和聚类的时间序列模块"""
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            adj_matrix = pairwise_distances(torch.Tensor(price_array).to(self.device),distance_func=self.criterion.ccc_distance_torch)
            # 如果维度不够，则补0
    ...
    def _compute_loss(self, output, target,optimizers_idx=0):
        ...
        return self.criterion(output,optimizers_idx=optimizers_idx)
