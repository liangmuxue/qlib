...
class TimeMixer(nn.Module):
    ...
    def __init__(self, c_in=10,c_out=1,seq_len=25, pred_len=5,past_cov_dim=12, dropout=0.3,decomp_method='moving_avg',d_ff=2048,moving_avg=25,
                 ...
                 num_nodes=0,node_dim=16,day_of_week_size=5,temp_dim_diw=8,month_of_year_size=12,temp_dim_miy=8,device="cpu"):
        """Params:
                c_in:输入目标维度
                c_out: 输出目标维度，默认1
                seq_len： 输入序列长度
    ...
    def forward(self, x_in): 
        ...
        x_enc, historic_future_covariates,future_covariates,past_round_target,past_index_target = x_in
        # 采样得到对应时间变量
        ...
        xpast_day_of_week,xpast_month_of_year = x_time_mark_past[...,0],x_time_mark_past[...,1]
        ...
        # 引入全局未来协变量，串接整体评估部分
        # all2index_data = dec_out[:,1:,:].reshape(dec_out.shape[0],-1)
        # dec_out[:,0,:] = dec_out[:,0,:] + self.all_to_index_projection_layer(all2index_data)
