...
class TideVaDE(nn.Module):
    ...
    def __init__(
        ...
        # 使用2个分别为均值和标准差的线性连接层，其中均值用于decode
        self.mu_layer = nn.Linear(hidden_size, z_layer_dim)
        self.sigma_layer = nn.Linear(hidden_size, z_layer_dim)
        
        self.decoders = nn.Sequential(
            # add decoder output layer
            _ResidualBlock(
                input_dim=z_layer_dim,
                output_dim=hidden_size,
                hidden_size=hidden_size,
                use_layer_norm=use_layer_norm,
                dropout=dropout,
    ...
    def forward(
        ...
        # 使用均值编码数据用于解码
        encoded_mu = self.mu_layer(encoded)     
        encoded_sigma = self.sigma_layer(encoded)   
        # 封装decode方法，透传x_lookback参数
        y,y_sig = self.decode_through(encoded_mu,x_lookback)
    ...
    def decode_through(self,encoded_mu,x_lookback):
        ...
        decoded = decoded.view(x_lookback.shape[0], x_lookback.shape[1],self.output_chunk_length, -1)
        ...
        y = y.view(-1, x_lookback.shape[1],self.output_chunk_length, self.output_dim)        
        y_sig = self.sig(y)
        return y,y_sig
