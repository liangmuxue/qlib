...
from cus_utils.kmeans_gpu import kmeans
...
def phi(feature, true_labels, cluster_num,device="cpu"):
    predict_labels, centers = kmeans(X=feature, num_clusters=cluster_num, distance="euclidean", device=device)
...
def high_confidence(Z, center,tao=0.9):
...
def pseudo_matrix(P, S, node_num,device=None,beta=1):
    P = torch.tensor(P)
...
class HsanLoss(_Loss):
    """封装相关损失函数"""

    def __init__(self,args={},ref_model=None,device="cpu"):
        super(HsanLoss, self).__init__()
        self.args = args
        self.ref_model = ref_model
        self.node_num = ref_model.n_num
        self.mask = torch.ones([self.node_num * 2, self.node_num * 2]) - torch.eye(self.node_num * 2)
        self.mask = self.mask.to(device)
        
    def forward(self, Z1, Z2, E1, E2):
        """合并聚类伪标签生成与损失计算"""
        
        # 聚类伪标签生成
        S = self.comprehensive_similarity(Z1, Z2, E1, E2, self.ref_model.alpha)
        
        # 损失计算
        pos_neg = self.mask * torch.exp(S * self.ref_model.pos_neg_weight)
        pos = torch.cat([torch.diag(S, self.node_num), torch.diag(S, - self.node_num)], dim=0)
        pos = torch.exp(pos * self.ref_model.pos_weight)
        neg = (torch.sum(pos_neg, dim=1) - pos)
