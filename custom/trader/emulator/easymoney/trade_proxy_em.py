...
from trader.emulator.easymoney.callback import callback_controller
from trader.emulator.easymoney.storage import ctx
...
from gm.api import Order,order_volume,get_unfinished_orders,order_cancel

from cus_utils.log_util import AppLogger
logger = AppLogger()

class EasymoneyOrder(object):
    """仿照掘金的订单类"""
...
class EmSpi(object):
    ...
    def find_cache_order(self,easymoney_order=None):
        """查找已生成订单
           Params: 
               easymoney_order 掘金订单对象信息
           Return:
               order 订单对象
               need_append boolean 是否需要添加到订单列表中
        """
        
        logger.debug("find_cache_order in, mode:{}".format(easymoney_order))
        # 匹配已存储订单
        for order in ctx.cached_orders:
            # 如果当前不包含掘金订单信息，则使用代码和价格查询
            if not hasattr(order,"easymoney_order"):
                order_book_id = self.transfer_symbol(easymoney_order.symbol, mode=1)
                if order.order_book_id==order_book_id and order.price==easymoney_order.price:
                    return order,True               
            else:
                if order.easymoney_order.cl_ord_id==easymoney_order.cl_ord_id:
                    return order,False
        return None,False
    ...
    # 委托状态变化时触发
    def on_order_status(self,easymoney_order):
        try:
            logger.info('order_stats_count:{}'.format(easymoney_order))
            # 已报单事件回调
            if easymoney_order.status==OrderStatus_New:
                order,need_append = self.find_cache_order(easymoney_order)
                if order is None:
                    logger.error("not found order:{}".format(easymoney_order))
                    return        
                # 放入掘金订单信息
                if need_append:
                    logger.debug("need append with second:{}".format(easymoney_order.cl_ord_id))
                    order.easymoney_order = easymoney_order    
                    # 订单中使用第二订单号来存储掘金订单编号
                    order.set_secondary_order_id(easymoney_order.cl_ord_id)
                order.active()
            ...
            # 成交事件回调
            if easymoney_order.status==OrderStatus_Filled:
                logger.debug("OrderStatus_Filled cl_ord_id find:{}".format(easymoney_order.cl_ord_id))
                order,_ = self.find_cache_order(easymoney_order)
                if order is None:
                    logger.error("not found order:{}".format(easymoney_order))
                    return               
                ...
                if hasattr(order,"execrpt"):
                    ...
                    trade = Trade.__from_create__(
                        ...
                        # 冻结价格取当前成交价格
                        frozen_price=easymoney_order.price,
                        # 当日可平仓位取0
                    ...
                    order.has_fill = True
            # 订单拒绝事件回调
            if easymoney_order.status==OrderStatus_Rejected:
            ...
            # 订单已撤单事件回调
            if easymoney_order.status==OrderStatus_Canceled:
                logger.debug("OrderStatus_Canceled cl_ord_id find:{}".format(easymoney_order.cl_ord_id))
                order,_ = self.find_cache_order(easymoney_order)
                if order is None:
                    logger.error("not found order:{}".format(easymoney_order))
                    return               
