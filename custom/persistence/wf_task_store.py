...
class WfTaskStore(BasePersistence):
    ...
    def get_workflow_details_config(self,workflow_id):
        ...
        sql = "select id,name,type,sequence,main_yaml_template,start_date,end_date," \
            "frequency,dynamic_config,extend_config,run_in_holiday from workflow_detail where workflow_id={} and del_flag=0 order by sequence asc".format(workflow_id)
        rows = self.dbaccessor.do_query(sql)
        rtn_obj = [{"id":row[0],"name":row[1],"type":row[2],
                    "sequence":row[3],"main_yaml_template":row[4],
                    "start_date":row[5],"end_date":row[6],"frequency":row[7],
                    "dynamic_config":row[8],"extend_config":row[9],"run_in_holiday":row[10]} for row in rows]
        return rtn_obj    
