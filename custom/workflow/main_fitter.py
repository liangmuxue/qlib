...
class MainFitter():
    ...
    def fit(
        ...
        if self.type=="backtest":
            ...
            # 保存文件用于回调进程
            df_data_path = self.kwargs["pred_data_path"] + "/df_all.pkl"
    ...
    def load_model(self,dataset,mode=None):        
        ...
        model = TFTCluSerModel.load_from_checkpoint(self.optargs["model_name"],work_dir=self.optargs["work_dir"],best=best_weight)
        model.batch_size = self.batch_size
        model.batch_file_path = self.kwargs["batch_file_path"]   
        model.mode = mode    
        self.model = model
        
        # 生成一阶段模型
        emb_size = dataset.get_emb_size()
        self.model_exp = self._build_model(dataset,emb_size=emb_size,use_model_name=True,mode=1,batch_file_path=self.kwargs["batch_file_path"] )   
        return self.model
